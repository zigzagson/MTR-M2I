...
def build_scheduler(optimizer, dataloader, opt_cfg, total_epochs, total_iters_each_epoch, last_epoch):
    ...
    def lr_lbmd(cur_iteration):
        cur_decay = 1
        for decay_step in decay_steps:
            if cur_iteration >= decay_step:
...
def build_dataloader(dataset_cfg, batch_size, workers=4,
...
def train_one_epoch(model, optimizer, train_loader, accumulated_iter, optim_cfg,
                    tbar, total_it_each_epoch, dataloader_iter, tb_log=None, leave_pbar=False, scheduler=None, show_grad_curve=False,
                    logger=None, logger_iter_interval=50, cur_epoch=None, total_epochs=None, ckpt_save_dir=None, ckpt_save_time_interval=300):


   #'/home/arclab/IV2023_first/output/Exp3/ckpt'
...
def train_model(model, optimizer, train_loader, optim_cfg,
                start_epoch, total_epochs, start_iter, ckpt_save_dir,
                ckpt_save_interval=1, max_ckpt_save_num=50, tb_log=None,
                scheduler=None, test_loader=None, logger=None, eval_output_dir=None, cfg=None, 
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            if (trained_epoch % ckpt_save_interval == 0 or trained_epoch in [1, 2, 4] or trained_epoch > total_epochs - 10):
            ...
            if test_loader is not None and (trained_epoch % ckpt_save_interval == 0 or trained_epoch in [1, 2, 4] or trained_epoch > total_epochs - 10):
                from tool.eval_utils.eval_utils import eval_one_epoch

                pure_model = model
                torch.cuda.empty_cache()
                tb_dict = eval_one_epoch(
                    cfg, pure_model, test_loader, epoch_id=trained_epoch, logger=logger, 
