#########I keeep the codes for detecting the agents that can only be seen
    ...
    def build_map(self, map_features, dynamic_map_states):
        ...
        for cur_data in map_features:
            ...
            if cur_data.lane.ByteSize() > 0:
                ...
                self.stop_signs_polylines[map_id]=cur_polyline
                self.polylines.append(cur_polyline)
                ...
                if len(cur_polyline) > 1:
                    ...
                    direction = np.array([0])[:, np.newaxis]
                cur_polyline = np.concatenate((cur_polyline[:, 0:3], cur_polyline_dir, cur_polyline[:, 3:],direction), axis=-1)            
                self.crosswalk_polylines[map_id] = cur_polyline
                self.polylines.append(cur_polyline)
                ...
                if len(cur_polyline) > 1:
                    ...
                    direction = np.array([0])[:, np.newaxis]
                cur_polyline = np.concatenate((cur_polyline[:, 0:3], cur_polyline_dir, cur_polyline[:, 3:],direction), axis=-1)            
                self.speed_bump_polylines[map_id] = cur_polyline
                self.polylines.append(cur_polyline)
                ...
                if len(cur_polyline) > 1:
                    ...
                    direction = np.array([0])[:, np.newaxis]
                cur_polyline = np.concatenate((cur_polyline[:, 0:3], cur_polyline_dir, cur_polyline[:, 3:],direction), axis=-1)      
                self.drive_way_polylines[map_id]=cur_polyline
                self.polylines.append(cur_polyline)     
            else:
                raise TypeError      

        try:
            self.polylines = np.concatenate(self.polylines, axis=0).astype(np.float32)  # [n,8]  [x,y,z,dx,dy,dz,type,theta]
        except:
    ...
    def process_data(self, save_path, viz=False):  
        ...
        for data_file in self.data_files:
            ...
            for data in dataset:
                ...
                info['traffic_light'] = self.traffic_light_info
