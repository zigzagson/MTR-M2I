...
import torch
import logging
from my_model_name.datasets.dataset import DatasetTemplate
from my_model_name.utils import common_utils
# from tool.visualization_input import visualization_input
...
class WaymoDataset(DatasetTemplate):
    ...
    def get_all_infos(self, info_path):
        ...
        infos = src_infos[::self.dataset_cfg['SAMPLE_INTERVAL'][self.mode]] # SAMPLE 
        self.logger.info(f'Total scenes: {len(infos)}')
    ...
    def create_scene_level_data(self, index):
        ...
        try:
            ...
            with open(self.data_path +'/'+f'sample_{scene_id}.npz', 'rb') as f:
                info = np.load(f,allow_pickle=True)

        sdc_track_index = info['sdc_track_index']
        current_time_index = 10
        timestamps = np.array(list(range(current_time_index+1)), dtype=np.float32)

        track_index_to_predict = np.array(info['predict_list'])
        obj_types = np.array(info['object_type'])
        obj_ids = np.array(info['object_id'])
        obj_trajs_full = info['all_agent'][:,:,:-1]                         # (num_objects, num_timestamp=91,F=10)
        obj_trajs_past = obj_trajs_full[:, :current_time_index + 1]         # (num_objects, num_timestamp=11,F=10)
        obj_trajs_future = obj_trajs_full[:, current_time_index + 1:]       # (num_objects, num_timestamp=80,F=10)
        ...
        # center_objects # (num_center_objects, num_attrs=10)
        # track_index_to_predict [index1,index2,...,indexn]
        ...
        (obj_trajs_data, obj_trajs_mask, obj_trajs_pos, obj_trajs_last_pos, obj_trajs_future_state, obj_trajs_future_mask, center_gt_trajs,
            center_gt_trajs_mask, center_gt_final_valid_idx, obj_types, obj_ids,track_index_to_predict_new) = self.create_agent_data_for_center_objects(
            ...
            timestamps=timestamps, obj_types=obj_types, obj_ids=obj_ids, intertest_agent_filter=info['interesting_index'] 
        )
        ...
        # print("-----------------------")
        # print(obj_trajs_data.shape)                #(num_center_objects, num_objects, num_timestamps=11, num_attrs=29)
        # print(obj_trajs_mask.shape)                #(num_center_objects, num_objects, num_timestamps=11)
        # print(obj_trajs_pos.shape)                 #(num_center_objects, num_objects, num_timestamps=11, 3)
        # print(obj_trajs_last_pos.shape)            #(num_center_objects, num_objects, 3)
        # print(obj_trajs_future_state.shape)        #(num_center_objects, num_objects, num_timestamps=80, num_attrs=4)              
        # print(obj_trajs_future_mask.shape)         #(num_center_objects, num_objects, num_timestamps=80)
        # print(center_gt_trajs.shape)               #(num_center_objects, num_timestamps=80, num_attrs=4)  
        # print(center_gt_trajs_mask.shape)          #(num_center_objects, num_timestamps=80)  
        # print(center_gt_final_valid_idx.shape)     # (num_center_objects,)
        # print(track_index_to_predict_new.shape)    # (num_center_objects,)
        # print(sdc_track_index_new)                 # int value
        # print(obj_types.shape)                     # (num_objects,)
        # print(obj_ids.shape)                       # (num_objects,)
        # print("--------------------------")
        ret_dict = {
            ...
            'origin_track_index_to_predict':track_index_to_predict,
            ...
            'center_objects_id': np.array(info['object_id'])[track_index_to_predict],
            'center_objects_type': np.array(info['object_type'])[track_index_to_predict],
        ...
        if info['all_polylines'].__len__() == 0:
            info['all_polylines'] = np.zeros((2, 7), dtype=np.float32)
        ...
        map_polylines_data, map_polylines_mask, map_polylines_center = self.create_map_data_for_center_objects(
            center_objects=center_objects, map_infos=info['all_polylines'],
            center_offset=self.dataset_cfg['CENTER_OFFSET_OF_MAP'],
            lane_info = info['polylines_info']
    ...
    def create_agent_data_for_center_objects(
            ...
            obj_types, obj_ids,intertest_agent_filter
        ):       
        ...
        max_agents = max(len(indices) for indices in intertest_agent_filter)
        filtered_obj_trajs_data = np.zeros((obj_trajs_data.shape[0], max_agents, obj_trajs_data.shape[2], obj_trajs_data.shape[3]))
        filtered_obj_trajs_mask = np.zeros((obj_trajs_data.shape[0], max_agents, obj_trajs_data.shape[2]))
        filtered_objs_future_state = np.zeros((obj_trajs_future_state.shape[0], max_agents, obj_trajs_future_state.shape[2], obj_trajs_future_state.shape[3]))
        filtered_obj_trajs_future_mask = np.zeros((obj_trajs_future_state.shape[0], max_agents, obj_trajs_future_state.shape[2]))
        for i in range(obj_trajs_data.shape[0]):
            filtered_obj_trajs_data[i,:len(intertest_agent_filter[i])] = obj_trajs_data[i, intertest_agent_filter[i]]
            filtered_obj_trajs_mask[i, :len(intertest_agent_filter[i])] =  obj_trajs_mask[i, intertest_agent_filter[i]]
            filtered_objs_future_state[i, :len(intertest_agent_filter[i])] = obj_trajs_future_state[i, intertest_agent_filter[i]]
            filtered_obj_trajs_future_mask[i, :len(intertest_agent_filter[i])] = obj_trajs_future_mask[i, intertest_agent_filter[i]] 

        obj_trajs_data = filtered_obj_trajs_data.astype(np.float32)
        obj_trajs_mask = filtered_obj_trajs_mask>0
        obj_trajs_future_state = filtered_objs_future_state.astype(np.float32)
        obj_trajs_future_mask = filtered_obj_trajs_future_mask>0
        track_index_to_predict_new =[]
        for indices in intertest_agent_filter:
            center_agent_track = indices[0]
            track_index_to_predict_new.append(indices.index(center_agent_track))
        ...
        return (obj_trajs_data, obj_trajs_mask > 0, obj_trajs_pos, obj_trajs_last_pos,
            ...
            obj_types, obj_ids,np.array(track_index_to_predict_new))
    
    # @staticmethod
    def get_interested_agents(self, track_index_to_predict, obj_trajs_full, current_time_index):
        center_objects_list = []
        ...
        for k in range(len(track_index_to_predict)):
            ...
            # assert obj_trajs_full[obj_idx, current_time_index, 9] > 0, f'obj_idx={obj_idx}, scene_id={scene_id}' # TODO: NO SELECTION
    ...
    def generate_centered_trajs_for_agents(self, center_objects, obj_trajs_past, obj_types, center_indices, sdc_index, timestamps, obj_trajs_future):
        ...
        )                                             # (num_center_objects,num_objects, 11, num_attrs)
    ...
    def create_map_data_for_center_objects(self, center_objects, map_infos, center_offset,lane_info):
        """
        Args:
            center_objects (num_center_objects, 10): [cx, cy, cz, dx, dy, dz, heading, vel_x, vel_y, valid]
            map_infos (dict):
        ...
        polylines = torch.from_numpy(map_infos.copy())[:lane_info[0]]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg_file', type=str, default="../tool/cfgs/waymo/mtr+100_percent_data.yaml", help='specify the config of dataset')
