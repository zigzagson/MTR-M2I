...
from my_model_name.models.utils.transformer import transformer_decoder_layer
from my_model_name.models.utils.transformer import position_encoding_utils
from my_model_name.models.utils import common_layers
from my_model_name.utils import common_utils, loss_utils, motion_utils
...
class MTRDecoder(nn.Module):
    ...
    def build_motion_query(self, d_model, use_place_holder=False):
        ...
        if use_place_holder:
            ...
            intention_points_file = "/home/zigzagson/Documents/ICRA2023/IV2023_first-Prediction/simulation/cluster_64_center_dict.pkl" # TODO:
    ...
    def apply_dense_future_prediction(self, obj_feature, obj_mask, obj_pos):
        # obj_feature [num_centered_objs,num_objects,512]
        # obj_mask    [num_centered_objs,num_objects]
        # obj_pos     [num_centered_objs,num_objects,3]        
        ...
        pred_dense_trajs_valid = torch.cat((temp_center, pred_dense_trajs_valid[:, :, 2:]), dim=-1) # [num_valid_objs,80,7]
    ...
    # center_objects_feature [num_center_objects,512]
    # center_object_types    [num_center_objects]
    # obj_feature            [num_center_objects,num_objects,512]
    # obj_mask               [num_center_objects,num_objects]
    # obj_pos                [num_center_objects,num_objects,3]
    # map_feature            [num_center_objects,num_polylines,256]
    # map_mask               [num_center_objects,num_polylines]
    # map_pos                [num_center_objects,num_polylines,3]
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            obj_query_feature = self.apply_cross_attention(               # (num_query, num_center_objects, 512)
            ...
            pred_scores = self.motion_cls_heads[layer_idx](query_content_t).view(num_center_objects, num_query) # [num_query, num_center_objects]
            ...
            pred_list.append([pred_scores, pred_trajs]) # [num_query, num_center_objects,80,7]
    ...
    def get_decoder_loss(self, tb_pre_tag=''):
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            pred_scores, pred_trajs = pred_list[layer_idx] # [num_center_objects, queries], [num_center_objects, queries, 80, 7] 
    ...
    def forward(self, batch_dict):
        ...
        # obj_feature    [num_centered_objs,num_objects,512]
        # pred_dense_future_trajs    [num_centered_objs,num_objects,80,7]
        ...
        if not self.training:
            ...
            batch_dict['intention_points']=self.forward_ret_dict['intention_points']
        else:
