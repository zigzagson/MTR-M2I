...
from my_model_name.models.utils.transformer import transformer_encoder_layer, position_encoding_utils
from my_model_name.models.utils import polyline_encoder
from my_model_name.utils import common_utils
from my_model_name.ops.knn import knn_utils
...
class MTREncoder(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        # obj_trajs          (num_centered_objects, num_objects, num_timestamps=11, F=29)
        # obj_trajs_mask     (num_centered_objects, num_objects, num_timestamps=11)
        

        map_polylines, map_polylines_mask = input_dict['map_polylines'].cuda(), input_dict['map_polylines_mask'].cuda() 
        # map_polylines (num_centered_objects, num_polylines, num_waypoints, F=9)
        # map_polylines_mask  (num_centered_objects, num_polylines, num_waypoints)
        ...
        global_token_feature = torch.cat((obj_polylines_feature, map_polylines_feature), dim=1)  # (num_center_objects, num_tokens,C=256)
        global_token_mask = torch.cat((obj_valid_mask, map_valid_mask), dim=1)                   # (num_center_objects, num_tokens)
        global_token_pos = torch.cat((obj_trajs_last_pos, map_polylines_center), dim=1)          # (num_center_objects, num_tokens,3)
        ...
        # print(center_objects_feature.shape)
